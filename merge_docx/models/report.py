...
class IrActionsReport(models.Model):
    ...
    def render_any_docs(self, res_ids=None, data=None):
        ...
        if full:
            ...
            if not output_file or output_file == 'docx':
                ...
                #Maged
                _doc = docx.Document(temp_full)
                _doc = self.tables_loop(_doc, docs)
                target_stream = BytesIO()
                _doc.save(target_stream)

                temp_full.close()
                
                #Maged
                target_stream.seek(0)
                out = target_stream.read()
    ...
    def delete_element(self, e):
        try:
            ...
            e._p = e._element = None
        except:
            p = e._element
            e._element = None

    #Maged
    def tables_loop(self, _doc, docs):
        i=0
                        ...
                        for cell in row.cells:
                            ...
                            try:
                                for paragraph in cell.paragraphs:
                                    ...
                                    if '[_loop_]' in paragraph.text:
                                        paragraph.text = paragraph.text.replace("[_loop_]","")

                                    if '[_One2many_o.' in paragraph.text:
                                        paragraph.text = paragraph.text.replace("[_One2many_","")
                                        paragraph.text = paragraph.text.replace("]","")
                                        paragraph.text = paragraph.text.replace('o.','')
                                        _arr = paragraph.text.split('.')
                                        for _e in _arr:
                                            _one2many_loop = _one2many_loop[_e]
                                            _on2many_p = paragraph
                                            paragraph.text = ''

                                    if '_if_p_' in paragraph.text:
                                        _before_arr = paragraph.text
                                        _before_arr = paragraph.text[:paragraph.text.find('[')]
                                        paragraph.text = paragraph.text[paragraph.text.find('['):]
                                        paragraph.text = paragraph.text.replace('[','').replace(']','').replace('o.','').replace('_if_p_','').replace(' ','')
                                        _arr = paragraph.text.split('.')
                                        for _e in _arr:
                                            if '_One2Many_' in _e and _e.count('_One2Many_') == 1:
                                                _e = _e.replace('_One2Many_','')
                                                try:
                                                    _prefix = _prefix[_e][i]
                                                except:
                                                    _delete_p = paragraph
                                                    # self.delete_element(_delete_p)
                                            elif '_One2Many_' in _e and _e.count('_One2Many_') >= 1:
                                                _list = _e.split('_One2Many_')
                                                try:
                                                    _prefix = _prefix[_list[1]][i]
                                                    _prefix = _prefix[_list[2]][i]
                                                except:
                                                    _delete_p = paragraph
                                                    # self.delete_element(_delete_p)
                                            elif '_Image_' in _e:
                                                _e = _e.replace('_Image_','')
                                                paragraph.text = ''
                                                _prefix = _prefix[_e]
                                                img_bytes = base64.b64decode(_prefix)
                                                image = io.BytesIO(img_bytes)
                                                r = paragraph.add_run()
                                                r.add_picture(image, width=Inches(1.25))
                                            elif '_Txt_' in _e:
                                                _e = _e.replace('_Txt_','')
                                                paragraph.text = _before_arr
                                                if not _prefix[_e]:
                                                    _delete_p = paragraph
                                                    # self.delete_element(_delete_p)
                                            else:
                                    ...
                                    elif '_if_table_' in paragraph.text:
                                        _before_arr = paragraph.text
                                        _before_arr = paragraph.text[:paragraph.text.find('[')]
                                        paragraph.text = paragraph.text[paragraph.text.find('['):]
                                        paragraph.text = paragraph.text.replace('[','').replace(']','').replace('o.','').replace('_if_table_','').replace('_if_p_','').replace(' ','')
                                        _arr = paragraph.text.split('.')
                                        for _e in _arr:
                                            if '_One2Many_' in _e and _e.count('_One2Many_') == 1 and not _delete_t:
                                                _e = _e.replace('_One2Many_','')
                                                try:
                                                    _prefix = _prefix[_e][i]
                                                except:
                                                    _delete_t = table
                                            elif '_One2Many_' in _e and _e.count('_One2Many_') >= 1 and not _delete_t:
                                                _list = _e.split('_One2Many_')
                                                try:
                                                    _prefix = _prefix[_list[1]][i]
                                                    _prefix = _prefix[_list[2]][i]
                                                    ...
                                                    _delete_t = table
                                                    self.delete_element(_delete_t)
                                            elif '_Image_' in _e and not _delete_t:
                                                _e = _e.replace('_Image_','')
                                                paragraph.text = ''
                                                _prefix = _prefix[_e]
                                                img_bytes = base64.b64decode(_prefix)
                                                ...
                                                r = paragraph.add_run()
                                                r.add_picture(image, width=Inches(1.25))
                                            elif '_Txt_' in _e and not _delete_t:
                                                _e = _e.replace('_Txt_','')
                                                paragraph.text = _before_arr
                                                if not _prefix[_e]:
                                                    _delete_t = table
                                    ...
                                    # for _rec in _one2many_loop:
                                    if '.' in paragraph.text and '[' in paragraph.text and ']' in paragraph.text:
                                                paragraph.text = paragraph.text[paragraph.text.find('['):]
                                                paragraph.text = paragraph.text.replace('[','').replace(']','').replace('o.','').replace(' ','')
                                                _arr = paragraph.text.split('.')
                                                for _e in _arr:
                                                    if '_One2Many_' in _e and _e.count('_One2Many_') == 1:
                                                        _e = _e.replace('_One2Many_','')
                                                        ...
                                                        _e = _e.replace('_Txt_','')
                                                        paragraph.text = ''
                                                        run = paragraph.add_run(_prefix[_e])
                                                        run.font.bold = False
                                                        run.font.size = Pt(12)
                                    ...
                                    if _on2many_p:
                                        self.delete_element(_on2many_p)
                                    if _delete_p:
                                        self.delete_element(_delete_p)
                            ...
                            self.nested_tables_loop(cell, docs, c)
                            c += 1
                        i += 1
                        if _delete_r:
                            self.delete_element(_delete_r)
                    except:
                        pass
                if _delete_t:
                    self.delete_element(_delete_t)
    ...
    #Maged
    def nested_tables_loop(self, _doc, docs, c):
        i=0
        for table in _doc.tables:
            _delete_t = False
            for row in table.rows:
                ...
                for cell in row.cells:
                    ...
                    for paragraph in cell.paragraphs:
                        ...
                        if '_if_p_' in paragraph.text:
                            ...
                            _before_arr = paragraph.text[:paragraph.text.find('[')]
                            paragraph.text = paragraph.text[paragraph.text.find('['):]
                            paragraph.text = paragraph.text.replace('[','').replace(']','').replace('o.','').replace('_if_p_','').replace('_if_p_','').replace(' ','')
                            _arr = paragraph.text.split('.')
                            for _e in _arr:
                                if '_One2Many_' in _e and _e.count('_One2Many_') == 1:
                                    _e = _e.replace('_One2Many_','')
                                    ...
                                    for _e in _arr:
                                        if '_One2Many_' in _e and _e.count('_One2Many_') == 1:
                                            ...
                                            for _rec in _prefix:
                                                ...
                                                # for __e in _arr:
                                                #     if '_Image_' in __e:
                                                #         __e = __e.replace('_Image_','')
                                                #         _prefix = _rec[__e]
                                                #         img_bytes = base64.b64decode(_prefix)
                                                #         image = io.BytesIO(img_bytes)
                                                #         _new_row_cell = table.add_row().cells
                                                #         _new_row_cell[0].add_picture(image, width=Inches(1.25))
                                                #     elif '_Txt_' in __e:
                                                #         __e = __e.replace('_Txt_','')
                                                #         _new_row_cell = table.add_row().cells
                                                #         _new_row_cell[0].add_paragraph(_rec[__e])
                                                _e = _arr[1] 
                                                if '_Image_' in _e:
                                                    _e = _e.replace('_Image_','')
                                                    paragraph.text = ''
                                            ...
                                            _list = _e.split('_One2Many_')
                                            _prefix = _prefix[_list[1]]
                                            for _rec in _prefix:
                                                for __rec in _rec[_list[2]]:
                                                    _delete_r = row
                                                    for __e in _arr:
                                                        if '_Image_' in __e:
                                                            __e = __e.replace('_Image_','')
                                                            ...
                                                            __e = __e.replace('_Txt_','')
                                                            _new_row_cell = table.add_row().cells
                                                            _new_row_cell[0].add_paragraph(__rec[__e])
                    ...
                    self.nested_tables_loop(cell, docs, _c)
                    _c += 1
                # i += 1
                if _delete_r:
                    self.delete_element(_delete_r)
