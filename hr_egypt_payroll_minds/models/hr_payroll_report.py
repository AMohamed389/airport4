...
class hr_payroll_report(models.Model):
    ...
    def init(self):
        query = """
            SELECT
                p.id as id,
                CASE WHEN True THEN 1 ELSE 0 END as count,
                CASE WHEN True THEN 0 ELSE 0 END as count_work,
                CASE WHEN True THEN 0 ELSE 0 END as count_work_hours,
                CASE WHEN 1 <> 0 THEN 0 ELSE 0 END as count_leave,
                CASE WHEN 1 = 0 THEN 0 ELSE 0 END as count_leave_unpaid,
                CASE WHEN True THEN 0 ELSE 0 END as count_unforeseen_absence,
                CASE WHEN True THEN 0 ELSE 0 END as leave_basic_wage,
                p.name as name,
                ...
                ,0 as work_code,
                CASE WHEN False IS NOT TRUE THEN '1' WHEN 1 = 0 THEN '3' ELSE '2' END as work_type,
                0 as number_of_days,
                0 as number_of_hours
                ,CASE WHEN True THEN pln.total ELSE 0 END as net_wage,
                CASE WHEN True THEN plb.total ELSE 0 END as basic_wage,
                CASE WHEN True THEN plg.total ELSE 0 END as gross_wage
            FROM
                (SELECT * FROM hr_payslip WHERE state IN ('done', 'paid')) p
                    ...
                    left join hr_payslip_line pln on (pln.slip_id = p.id and  pln.code = 'NSAT')
                    left join hr_payslip_line plb on (plb.slip_id = p.id and plb.code = 'BASIC_EG')
                    left join hr_payslip_line plg on (plg.slip_id = p.id and plg.code = 'GROSS_EG')
            GROUP BY
                e.id,
                e.department_id,
                e.company_id,
